...
MongoClient.connect(mongoUrl, { serverSelectionTimeoutMS: 5000 })
    .then(async client => {
        ...
        // Só rehash a senha se o admin não existir
        if (!adminUser) {
            console.log('Configurando usuário "admin" inicial...');
            const hashedPassword = await bcrypt.hash('stl2025', 10);
            await users.updateOne(
                { username: 'admin' },
                { $set: { username: 'admin', password: hashedPassword, role: 'admin', apartments: [] } },
                { upsert: true }
            );
