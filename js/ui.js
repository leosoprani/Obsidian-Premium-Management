...
export function updateApprovalsBadge() {
    ...
    badge.classList.toggle('hidden', pendingApprovals.length === 0);
}
            ...
            <table class="w-full text-sm text-left min-w-[960px]">
                ...
                <tbody>
                    ${guests.sort((a, b) => a.name.localeCompare(b.name)).map(guest => {
                        ...
                        return `
                            <tr class="border-b">
                                <td class="px-4 py-2 whitespace-nowrap">${guest.name}</td>
                    ...
                    <tbody>
                        ${userList.sort((a, b) => a.username.localeCompare(b.username)).map(user => `
                            <tr class="border-b">
                                ...
                                <td class="p-4 text-on-surface-variant">${user.role === 'owner' ? user.apartment || 'Nenhum' : '-'}</td>
                                <td class="p-4">
                                    <div class="flex gap-2">
                                        ${user.username !== 'admin' ? `
...
function createOwnerDropdownMenus() {
    const dropdownContent = `
        ...
        <a href="#" id="owner-exit-btn" class="flex items-center gap-3 px-3 py-2 text-sm rounded-md text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20"><i data-lucide="log-out" class="w-4 h-4"></i>Sair</a>
    `;
...
export function renderOwnerPortal(appState) {
    const { properties, currentUser } = appState;
    const ownerApartment = properties[0]; // Proprietário só tem acesso a um apartamento

    if (!ownerApartment) {
        document.getElementById('owner-portal').innerHTML = '<div class="card p-8 text-center">Apartamento não encontrado ou não vinculado a este usuário.</div>';
        return;
    }
    ...
    document.getElementById('owner-apartment-name').textContent = `Apartamento: ${ownerApartment.name}`;

    // Cria os menus dropdown agora que o portal está visível
    createOwnerDropdownMenus();

    // Não renderiza mais o calendário, apenas o dashboard e a lista de reservas
    ...
    // Usa o termo de busca que não estiver vazio
...
function renderOwnerDashboard(appState) {
    const { properties, reservations, currentDate } = appState;
    const container = document.getElementById('owner-dashboard-stats');
    if (!container) return;

    const ownerApartment = properties[0];
    ...
    const confirmedReservations = reservations.filter(res => 
        res.apartment === ownerApartment.name &&
...
function renderOwnerMonthlyOccupancyChart(appState) {
    ...
    container.innerHTML = `
        <h3 class="text-lg font-semibold mb-4">Ocupação Diária (Mês Atual)</h3>
    ...
    reservations.forEach(res => {
        if (res.apartment !== ownerApartment.name || (res.status !== 'confirmed' && res.status !== 'checked-in')) return;
...
function renderOwnerRevenueChart(appState) {
    const { properties, reservations } = appState;
    const container = document.getElementById('owner-revenue-chart-container');
    if (!container) return;

    container.innerHTML = `
        <h3 class="text-lg font-semibold mb-4">Receita por Mês (Últimos 6 meses)</h3>
        <div class="relative h-64">
...
export function renderOwnerReservationsList(appState, searchTerm = '') {
    const { reservations, guests } = appState;
    ...
    let ownerReservations = reservations.filter(r => r.guestId !== 'TASK');
...
export function renderPendingApprovals(reservations, guests) {
    ...
    const pending = reservations.filter(res => res.status === 'pending-approval').sort((a, b) => new Date(a.checkin) - new Date(b.checkin));

    container.innerHTML = '';
    if (pending.length === 0) {
        container.innerHTML = `<p class="text-outline col-span-full text-center">Nenhuma reserva aguardando aprovação.</p>`;
        return;
    }
    ...
    pending.forEach(res => {
        ...
        const cardHtml = `
            <div class="card p-4 flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-start">
                        <span class="text-xs font-bold py-1 px-2 rounded-full bg-yellow-100 text-yellow-800">Aguardando Aprovação</span>
                        <span class="text-lg font-bold py-1 px-3 rounded-full" style="background-color: var(--secondary-bg); color: var(--text-color);">Apto ${res.apartment}</span>
                    </div>
                    <div class="mt-3">
                        <h3 class="text-lg font-semibold">${guest.name}</h3>
                        <p class="text-sm text-outline">${guest.phone}</p>
                    </div>
                    <div class="text-sm text-on-surface-variant mt-2 border-t pt-2">
                        <p><strong>Check-in:</strong> ${checkin.toLocaleDateString('pt-BR')}</p>
                        <p><strong>Check-out:</strong> ${checkout.toLocaleDateString('pt-BR')}</p>
                        <p><strong>Noites:</strong> ${nights}</p>
                        ${res.notes ? `
                            <div class="mt-2 pt-2 border-t border-dashed">
                                <p><strong>Observações:</strong></p>
                                <p class="text-outline italic">"${res.notes}"</p>
                            </div>
                        ` : ''}
                    </div>
                </div>
                <div class="flex gap-2 mt-4">
                    <button class="btn btn-secondary flex-1" onclick="window.app.handlers.rejectReservationHandler('${res.id}')">
                        <i data-lucide="x" class="w-4 h-4"></i> Recusar
                    </button>
                    <button class="btn flex-1" style="background-color: var(--brand-green); color: white;" onclick="window.app.handlers.approveReservationHandler('${res.id}')">
                        <i data-lucide="check" class="w-4 h-4"></i> Aprovar
                    </button>
                </div>
                <button class="btn btn-ghost w-full mt-2 text-sm" onclick="window.app.modals.openReservationModal('${res.id}')">
                    Ver / Editar Detalhes
                </button>
            </div>
        `;
