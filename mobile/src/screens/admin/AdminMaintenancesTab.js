...
export default function AdminMaintenancesTab({ onBack }) {
  ...
  const renderItem = ({ item }) => (
    <View style={[styles.card, { backgroundColor: activeTheme.colors.glass, borderColor: activeTheme.colors.glassBorder, borderWidth: 1 }]}>
      <View style={[styles.iconBox, { backgroundColor: `${activeTheme.colors.primary}15` }]}>
        <Image source={require('../../../assets/icons/repair_white.png')} style={{ width: 22, height: 22 }} resizeMode="contain" />
      </View>
      <TouchableOpacity 
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <Image source={require('../../../assets/icons/repair_white.png')} style={{ width: 48, height: 48, opacity: 0.3 }} resizeMode="contain" />
            <Text style={[styles.emptyText, { color: activeTheme.colors.text }]}>Nenhuma manutenção registrada.</Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  card: { 
    ...
    padding: 18, borderRadius: 30, marginBottom: 16, borderWidth: 0,
    overflow: 'hidden',
  },
