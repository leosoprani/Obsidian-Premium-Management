...
export default function AdminGuestsTab({ onBack }) {
  ...
  const renderItem = ({ item }) => (
    <View style={[styles.card, { borderColor: activeTheme.colors.glassBorder, backgroundColor: activeTheme.colors.glass }]}>
        <View style={[styles.iconBox, { backgroundColor: activeTheme.colors.primary + '20' }]}>
            <Text style={[styles.initials, { color: activeTheme.colors.primary }]}>{(item.name || '?').substring(0, 2).toUpperCase()}</Text>
        </View>
        <View style={styles.details}>
            <Text style={[styles.name, { color: activeTheme.colors.text }]}>{item.name}</Text>
            <Text style={[styles.info, { color: activeTheme.colors.textTertiary }]}>{item.docType || 'DOC'}: {item.doc || '—'}</Text>
            {item.phone && <Text style={[styles.info, { color: activeTheme.colors.textTertiary }]}>{item.phone}</Text>}
        </View>
        <TouchableOpacity style={styles.actionBtn}>
            <Image source={require('../../../assets/icons/chevron_forward_inactive.png')} style={{ width: 14, height: 14, tintColor: activeTheme.colors.textSecondary, opacity: 0.3 }} resizeMode="contain" />
        </TouchableOpacity>
    </View>
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: activeTheme.colors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
            <Text style={[styles.title, { color: activeTheme.colors.text }]}>Hóspedes</Text>
            <Text style={[styles.sub, { color: activeTheme.colors.primary }]}>BASE DE CLIENTES</Text>
        </View>
      </View>

      <View style={styles.searchContainer}>
          <View style={[styles.searchBar, { borderColor: activeTheme.colors.glassBorder, backgroundColor: activeTheme.colors.glass }]}>
              <Image source={require('../../../assets/icons/search_inactive.png')} style={{ width: 18, height: 18, marginRight: 10, tintColor: activeTheme.colors.textSecondary, opacity: 0.5 }} resizeMode="contain" />
              <TextInput 
        ...
        ListEmptyComponent={
            <View style={styles.empty}>
                <Image source={require('../../../assets/icons/people_inactive.png')} style={{ width: 48, height: 48, tintColor: activeTheme.colors.textTertiary, opacity: 0.5 }} resizeMode="contain" />
                <Text style={[styles.emptyText, { color: activeTheme.colors.textTertiary }]}>Nenhum hóspede encontrado.</Text>
            </View>
        }
      />
    </View>
